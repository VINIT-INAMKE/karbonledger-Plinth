...
--
-- NOTE: PlutusTx traceError\/traceIfFalse emit trace messages via Builtins.trace,
-- but the thrown exception is a generic PlutusTx.Builtins.Internal.error without
-- the error code in the message. Verifying specific error codes requires a Plutus
-- emulator with trace capture (see V2-03). For now we verify rejection only.
testAttackRejected2
  :: String
...
--
-- See note on 'testAttackRejected2' regarding error code verification limitations.
testAttackRejected3
  :: String
